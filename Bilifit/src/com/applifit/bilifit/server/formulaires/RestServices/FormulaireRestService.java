...
public class FormulaireRestService {
	...
	@Path("/cookie")
	@Produces("application/json")
	public String getCookie(@HeaderParam("host") String host,
	...
	@Consumes("application/json")
	public Response createFormulaireComplet(Formulaire formulaire,
	...
	@Produces("application/json")
	public Formulaire getFormParNomAndVersion(@PathParam("nom") String nom,
	...
	@Produces("application/json")
	public List<Formulaire> getFormsByEntreprise(
			@PathParam("entreprise") long entreprise) {
		return fdao.getFormulairesByEntreprise(entreprise);
	}

	/**
	 * Récuperer les formulaires correspondants à une recherche par nom et
	 * date
	 * 
	 * @param entreprise
	 * @param nom
	...
	@Produces("application/json")
	public List<Formulaire> getFormsByParam(
			@PathParam("entreprise") long entreprise,
			@PathParam("nom") String nom, @PathParam("date") String date) {
	...
	@Produces("application/json")
	public Boolean getCountFormulaireByEntreprise(
	...
	@Path("/creerupload/")
	@Consumes("application/json")
	public Response createuploadlien() {
		String str = blobstoreService.createUploadUrl("/formulaire/upload");
		String str1 = str.replace("/", "3x4x5x6x");
		return Response.created(URI.create("/formulaire/uploadimg/" + str1))
				.build();
	}
	...
	@Path("/upload/")
	@Consumes("application/json")
	public Response uploadlien(Map<String, List<BlobKey>> blobs) {
		...
		return Response.created(URI.create(("/formulaire/uploadimg/" + str + "aaaaa1")))
		.build();

	}
		String str =blobKey.getKeyString().replace("/", "3x4x5x6x");
	return 	Response.created(URI.create(("/formulaire/uploadimg/" + str + "aaaaa1")))
				.build();
	}
	/**
	...
	@Produces("application/json")
	public List<Formulaire> getFormsVersionByCompte(
			@PathParam("compte") long compte) {
	...
	@Consumes("application/json")
	public Response allouer(@PathParam("compte") Long compte,
			@PathParam("formulaire") Long formulaire) {
	...
	@Produces("application/json")
	public List<Compte> getAllocationUsers(
			@PathParam("formulaire") Long formulaire) {
	...
	@Produces("application/json")
	public List<Formulaire> getFormsCompteByParam(
			@PathParam("compte") long compte, @PathParam("nom") String nom,
	...
	public Response addForm(String form) throws JSONException {
		...
		Formulaire formulaire = new Formulaire(
				formulaireObject.getString("nom"),
				formulaireObject.getString("commentaire"), 1,
				formulaireObject.getString("date_creation"),
				new Key<Entreprise>(Entreprise.class, formulaireObject
						.getLong("entreprise")), null);
		...
		while (formulaireObject.has("element" + i)) {
			...
			Element e = new Element(elementObject.getString("type"),
					elementObject.getInt("position_x"),
					elementObject.getInt("position_y"), new Key<Formulaire>(
							Formulaire.class, formulaire.getId()));
			e = edao.ajouterElement(e);
			int k = 0;
			while (formulaireObject.has("parametre" + i + k)) {
				paramObject = formulaireObject.getJSONObject("parametre" + i
						+ k);
				Parametre p = new Parametre(paramObject.getString("nom"),
						paramObject.getString("valeur"), new Key<Element>(
								Element.class, e.getId()));
		...
		while (formulaireObject.has("user" + i)) {
			fdao.allouerForm(formulaireObject.getLong("user" + i),
					formulaire.getId());
			i++;
		}
	...
	public Response apdateForm(String form) throws JSONException {
		...
		Formulaire formulaire = new Formulaire(
				formulaireObject.getString("nom"),
				formulaireObject.getString("commentaire"),
				formulaireObject.getInt("version"),
				formulaireObject.getString("date_creation"),
				new Key<Entreprise>(Entreprise.class, formulaireObject
						.getLong("entreprise")), null);
	...
	@Consumes("application/json")
	public Response changeAllocation(@PathParam("id") Long id, String comptes)
